...
scRNA_Fib=readRDS('./scRNA_Fib_new.rds')
...
scRNA_other = subset(scRNA, celltype %in% c('Endothelial','Melanocyte'))
scRNA_other$subcelltype = scRNA_other$celltype

scRNAsub <- merge(scRNA_Fib, y=c(scRNA_T,scRNA_Myeloid,scRNA_KRT,scRNA_other))
...
mat2[c(12),] <- mat[c(12),] #感兴趣的是第3行细胞（T细胞）
pdf(file=paste0('Healthy_KRT_cellchat_weight.pdf'),width = 7,height = 7)
...
##指定信号通路绘制气泡图
pdf(file=paste0('Healthy_Fib_bubble.pdf'),width = 6,height = 3)
netVisual_bubble(cellchat,
                 sources.use = c(11),
                 targets.use = c(1:10,12,14:19,21),
...
pdf(file=paste0('Melanoma_Endo2_cellchat_weight.pdf'),width = 6,height = 6)
...
levels(cellchat@idents)   #查看细胞亚群及factor顺序
vertex.receiver = c(8:13)  #左侧列展示感兴趣的亚群
#层级图(Hierarchy plot)/和弦图
pdf(file=paste0('Melanoma_Endo2_CXCL_Hierarchy.pdf'),width = 10,height = 7)
netVisual_aggregate(cellchat,                #左侧列展示感兴趣的亚群
                    layout = c('hierarchy'), #"circle", "hierarchy", "chord"
                    signaling = pathways.show,
                    vertex.receiver = vertex.receiver)
...
#计算配受体对在目标信号通路中的贡献条形图
pdf(file=paste0('Melanoma_CXCL_MIF_contribution.pdf'),width = 4,height = 3)
netAnalysis_contribution(cellchat, signaling = pathways.show)
dev.off()
#提取细胞对
pairLR.CXCL <- extractEnrichedLR(cellchat,
                                 signaling = pathways.show,
                                 geneLR.return = FALSE)
LR.show <- pairLR.CXCL[3,] #以贡献度top1的配受体对为例
pairLR.CXCL;LR.show
#Hierarchy plot
netVisual_individual(cellchat,
                     layout = c('hierarchy'),
...
#c()里面的数字与levels(cellchat@idents)中细胞的位次对应
##指定信号通路绘制气泡图
pdf(file=paste0('Melanoma_KRT_bubble.pdf'),width = 5,height = 2.5)
netVisual_bubble(cellchat,
                 sources.use = c(5),
                 targets.use = c(3:4,7:14,21,22),
                 signaling = c("MIF","CXCL","MK","PTN","PARs"), #指定信号通路
                 remove.isolate = FALSE) +
  theme(panel.grid = element_blank(),
        axis.text.x=element_text(angle=45,hjust = 1,vjust=1))
dev.off()
...
#创建一个空矩阵，将想要分析的细胞所在行填充进去
mat2 <- matrix(0, nrow = nrow(mat), ncol = ncol(mat), dimnames = dimnames(mat))
mat2[c(10),] <- mat[c(10),] #感兴趣的是第3行细胞（T细胞）
pdf(file=paste0('Vitiligo_Endo_cellchat_weight.pdf'),width = 6,height = 6)
netVisual_circle(mat2, vertex.weight = groupSize,arrow.size = 0.2, weight.scale = T, edge.weight.max = max(mat), title.name = rownames(mat)[5])
dev.off()
...
pathways.show <- c('MIF')   #以'MIF'信号通路展示为例
levels(cellchat@idents)   #查看细胞亚群及factor顺序
vertex.receiver = c(3,8)  #左侧列展示感兴趣的亚群
#层级图（Hierarchy plot）
pdf(file=paste0('Vitiligo_CD8T_MIF_层级图.pdf'),width = 10,height = 7)
netVisual_aggregate(cellchat,                #左侧列展示感兴趣的亚群
                    layout = c('hierarchy'), #"circle", "hierarchy", "chord"
                    signaling = pathways.show,
                    vertex.receiver = vertex.receiver)
...
#c()里面的数字与levels(cellchat@idents)中细胞的位次对应
##指定信号通路绘制气泡图
pdf(file=paste0('Vitiligo_Endo_bubble.pdf'),width = 6,height = 4)
netVisual_bubble(cellchat,
                 sources.use = c(22),
                 targets.use = c(2,5,8:21),
                 #signaling = c("CCL","CXCL","IFN-II"), #指定信号通路
                 remove.isolate = FALSE) +
  theme(panel.grid = element_blank(),
        axis.text.x=element_text(angle=45,hjust = 1,vjust=1))
