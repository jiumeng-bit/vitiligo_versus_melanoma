...
##合并T细胞与其他细胞
scRNA_other = subset(scRNA, celltype == c('Endothelial'))
scRNA_other$subcelltype = scRNA_other$celltype

Mela_celltype=c('Tumor','Normal','Tumor','Normal','Tumor','Tumor') 
...
scRNA_Mela$Mela_celltype <- Idents(scRNA_Mela)
...
scRNA_Fib$subcelltype = scRNA_Fib$Fib_celltype_stress
scRNA_KRT$subcelltype = scRNA_KRT$KRT_celltype

scRNAsub <- merge(scRNA_Mela, y=c(scRNA_other,scRNA_T,scRNA_Myeloid,scRNA_Fib,scRNA_KRT))
...
mat2[c(6,9),] <- mat[c(6,9),] #感兴趣的是第3行细胞（T细胞）
pdf(file=paste0('Healthy_Melanocyte_cellchat_weight.pdf'),width = 7,height = 7)
...
pathways.show <- c('CXCL')   #以'MIF'信号通路展示为例
levels(cellchat@idents)   #查看细胞亚群及factor顺序
vertex.receiver = c(1:8,12)  #左侧列展示感兴趣的亚群
#层级图（Hierarchy plot）
pdf(file=paste0('Healthy_Melanocyte_CXCL_层级图.pdf'),width = 10,height = 7)
...
pdf(file=paste0('Healthy_Melanocyte_CXCL_贡献条形图.pdf'),width = 4,height = 3)
...
##指定信号通路绘制气泡图
pdf(file=paste0('Healthy_Melanocyte_bubble.pdf'),width = 6,height = 3)
netVisual_bubble(cellchat,
                 sources.use = c(10),
                 targets.use = c(1:9,11:12),
...
pdf(file=paste0('Healthy_Melanocyte_CCL_expression.pdf'),width = 6,height = 4.5)
...
#创建一个空矩阵，将想要分析的细胞所在行填充进去
mat2 <- matrix(0, nrow = nrow(mat), ncol = ncol(mat), dimnames = dimnames(mat))
mat2[c(9),] <- mat[c(9),] #感兴趣的是第3行细胞（T细胞）
pdf(file=paste0('Melanoma_fromMel_cellchat_weight.pdf'),width = 6,height = 6)
netVisual_circle(mat2, vertex.weight = groupSize,arrow.size = 0.2, weight.scale = T, edge.weight.max = max(mat), title.name = rownames(mat)[5])
dev.off()
...
levels(cellchat@idents)   #查看细胞亚群及factor顺序
vertex.receiver = c(5,6)  #左侧列展示感兴趣的亚群
#层级图（Hierarchy plot）
pdf(file=paste0('Melanoma_Melanocyte_CXCL_层级图.pdf'),width = 10,height = 7)
netVisual_aggregate(cellchat,                #左侧列展示感兴趣的亚群
                    layout = c('hierarchy'), #"circle", "hierarchy", "chord"
                    signaling = pathways.show,
                    vertex.receiver = vertex.receiver)
...
#计算配受体对在目标信号通路中的贡献条形图
pdf(file=paste0('Melanoma_Melanocyte_CXCL_贡献条形图.pdf'),width = 4,height = 3)
netAnalysis_contribution(cellchat, signaling = pathways.show)
dev.off()
#提取细胞对
pairLR.CXCL <- extractEnrichedLR(cellchat,
...
#c()里面的数字与levels(cellchat@idents)中细胞的位次对应
##指定信号通路绘制气泡图
pdf(file=paste0('Melanoma_fromMel_bubble.pdf'),width = 4.5,height = 3)
netVisual_bubble(cellchat,
                 sources.use = c(9),
                 targets.use = c(1:7),
                 signaling = cellchat@netP$pathways[1:5], #指定信号通路
                 remove.isolate = FALSE) +
  theme(panel.grid = element_blank(),
        axis.text.x=element_text(angle=45,hjust = 1,vjust=1))
dev.off()
...
#参与目标信号通路的基因在各细胞亚群的表达分布展示
pdf(file=paste0('Melanoma_Melanocyte_CXCL_expression.pdf'),width = 6,height = 4.5)
...
#创建一个空矩阵，将想要分析的细胞所在行填充进去
mat2 <- matrix(0, nrow = nrow(mat), ncol = ncol(mat), dimnames = dimnames(mat))
mat2[,c(19)] <- mat[,c(19)] #感兴趣的是第3行细胞（T细胞）
pdf(file=paste0('Vitiligo_Mel_cellchat_weight.pdf'),width = 6,height = 6)
netVisual_circle(mat2, vertex.weight = groupSize,arrow.size = 0.2, weight.scale = T, edge.weight.max = max(mat), title.name = rownames(mat)[5])
dev.off()
...
pathways.show <- c('CXCL')   #以'MIF'信号通路展示为例
levels(cellchat@idents)   #查看细胞亚群及factor顺序
vertex.receiver = c(18,22)  #左侧列展示感兴趣的亚群
#层级图（Hierarchy plot）
pdf(file=paste0('Vitiligo_Melanocyte_CXCL_层级图.pdf'),width = 10,height = 7)
netVisual_aggregate(cellchat,                #左侧列展示感兴趣的亚群
                    layout = c('circle'), #"circle", "hierarchy", "chord"
                    signaling = pathways.show,
                    vertex.receiver = vertex.receiver)
dev.off()

#计算配受体对在目标信号通路中的贡献条形图
pdf(file=paste0('Vitiligo_Melanocyte_CXCL_贡献条形图.pdf'),width = 4,height = 3)
netAnalysis_contribution(cellchat, signaling = pathways.show)
dev.off()
#提取细胞对
pairLR.CXCL <- extractEnrichedLR(cellchat,
...
#c()里面的数字与levels(cellchat@idents)中细胞的位次对应
##指定信号通路绘制气泡图
pdf(file=paste0('Vitiligo_Melanocyte_bubble.pdf'),width = 6.5,height = 4)
netVisual_bubble(cellchat,
                 sources.use = c(3:23),
                 targets.use = c(2),
                 #signaling = c("CCL","CXCL","IFN-II"), #指定信号通路
...
pairLR.use
netVisual_bubble(cellchat,
                 sources.use = c(1:4,7:9),
                 targets.use = c(5),
                 pairLR.use = pairLR.use,
                 remove.isolate = TRUE) + coord_flip()

#参与目标信号通路的基因在各细胞亚群的表达分布展示
pdf(file=paste0('Vitiligo_Melanocyte_GALECTIN_expression.pdf'),width = 6,height = 4)
